...
public class dfDynamicFont : dfFontBase
{
	...
	private void getGlyphData( CharacterInfo[] result, string text, int size, FontStyle style )
	{
		...
		{
			glyphBuffer = new CharacterInfo[ text.Length + 512 ];
		}
			...
			try
			{
				...
				if( isTextToken )
				{
					...
					for( int i = 0; i < token.Length; i++ )
					{
						...
						totalWidth += ( ch != ' ' )
							? Mathf.CeilToInt( glyph.vert.x + glyph.vert.width )
							: Mathf.CeilToInt( glyph.width );

						// Account for character spacing
						if( i > 0 )
							totalWidth += Mathf.CeilToInt( CharacterSpacing * TextScale );
